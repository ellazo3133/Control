...
function AddEmployeeModal({onClose,onSave}){
  ...
  const [salary,setSalary]=useState('');const [sched,setSched]=useState({});
  const [loading,setLoading]=useState(false);const [err,setErr]=useState('');
  const doSave=async()=>{
    if(!name||!email||!pw)return setErr('Completá nombre, email y contraseña');
    setLoading(true);setErr('');
    try{await onSave(name,email,pw,parseFloat(salary)||0,sched);onClose();}
...
export default function AdminView({profile,onLogout}){
  ...
  // Add employee
  const handleAddEmployee=async(name,email,pw,salary,sched)=>{
    ...
    if(rpcError){
      ...
      empId=rpcData;
      if(salary>0)await supabase.from('profiles').update({salary}).eq('id',empId);
    }
